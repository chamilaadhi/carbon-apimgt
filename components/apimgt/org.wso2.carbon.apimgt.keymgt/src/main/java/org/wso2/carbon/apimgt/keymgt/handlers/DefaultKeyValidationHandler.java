...
public class DefaultKeyValidationHandler extends AbstractKeyValidationHandler {
    ...
    private boolean isAccessTokenExpired(long validityPeriod, long issuedTime) {
        ...
        long timestampSkew = ServiceReferenceHolder.getInstance().getOauthServerConfiguration()
                .getTimeStampSkewInSeconds() * 1000L;

        long adjustedCurrentTime = System.currentTimeMillis() - timestampSkew;

        // Only evaluate expiry if token is not set to never expire
        if (validityPeriod != Long.MAX_VALUE) {
            // Prevent overflow by ensuring adjustedCurrentTime is reasonably smaller
            if (adjustedCurrentTime > validityPeriod && adjustedCurrentTime > (issuedTime + validityPeriod)) {
                return true;
            }
        }
        ...
        if (cachedAccessTokenInfo instanceof AccessTokenInfo) {
            ...
            if (isAccessTokenExpired(cachedAccessTokenInfoObject.getValidityPeriod(),
                ...
                if (log.isDebugEnabled()) {
                    log.debug("Invalid OAuth Token in Introspect Cache : Access Token " + APIUtil.getMaskedToken(
                            validationContext.getAccessToken()) + " has been expired.");
                }
